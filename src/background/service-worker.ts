...
async function route(
  ...
  switch (msg.type) {
    case 'SAVE_WALKTHROUGH': {
      ...
      if (!parsed.success) return err(`Invalid walkthrough: ${parsed.error.message}`)
    ...
    case 'LIST_WALKTHROUGHS': {
      const origin = (msg.payload as { origin?: string })?.origin ?? ''
      const list = origin
    ...
    case 'SET_PROGRESS': {
      const p = msg.payload as { walkthroughId?: string; stepIndex?: number; savedAt?: number }
      if (!p?.walkthroughId || tabId == null) return err('Missing required fields')
      await setProgress({
    ...
    case 'GET_PROGRESS': {
      if (tabId == null) return err('No tabId in sender')
      return ok(await getProgress(tabId))
    }

    case 'CLEAR_PROGRESS': {
      if (tabId != null) await clearProgress(tabId)
      return ok(null)
    }
...
async function setBadgeForUrl(tabId: number, url?: string): Promise<void> {
  ...
  try {
    ...
    await chrome.action.setBadgeText({ text, tabId })
    if (text) await chrome.action.setBadgeBackgroundColor({ color: '#3b82f6', tabId })
  } catch {}
}
