...
export function generateSelectors(el: HTMLElement): SelectorCandidate[] {
...
export function extractTextHint(el: HTMLElement): string | undefined {
  const text = el.textContent?.trim()
  if (!text || text.length > 100 || /\d{4,}/.test(text)) return undefined
...
function isAutoId(id: string): boolean {
  return (
    /^:\w+:$/.test(id)        || // React 18 hydration ids (:r0:)
...
function inferImplicitRole(el: HTMLElement): string | null {
  const tag = el.tagName.toLowerCase()
...
function buildStructuralPath(el: HTMLElement): string | null {
  const MAX_DEPTH = 4
  const parts: string[] = []
  ...
  while (current.parentElement && depth < MAX_DEPTH) {
    ...
    const parentHasStable =
      parent.id ||
      STABLE_DATA_ATTRS.some(a => parent.getAttribute(a))
    if (ANCHOR_TAGS.has(tag) || parentHasStable) break
