...
export function resolveElement(
  ...
  for (const c of sorted) {
    if (c.selector === primary) continue
...
function tryCss(selector: string, textHint?: string): HTMLElement | null {
  ...
  if (textHint) {
    ...
    for (const el of Array.from(matches)) {
      if (el.textContent?.trim().toLowerCase().startsWith(prefix)) return el
    }
  }
  return matches[0]
}

function scanByText(tagHint: string, textHint: string): HTMLElement | null {
  const prefix = textHint.slice(0, 30).toLowerCase()
  try {
    const els = Array.from(document.querySelectorAll<HTMLElement>(tagHint))
    for (const el of els) {
      if (el.textContent?.trim().toLowerCase().startsWith(prefix)) return el
    }
  } catch {}
