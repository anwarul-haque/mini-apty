...
let spaUnlisten = null;
let triggerCleanup = null;
let highlightCleanup = null;
// ── Public API ────────────────────────────────────────────────────────────────
export function startPlayback(wt, initialStep = 0) {
    cleanupAll();
...
function renderCurrentStep() {
    ...
    triggerCleanup = null;
    watchHandle?.stop();
    watchHandle = watchForElement(() => resolveElement(step.selector, step.candidates, {
        textHint: step.textHint,
        tagHint: step.tagHint,
    }), (el) => { attachHighlight(el); renderTooltip(el, step); }, () => renderErrorTooltip(step));
}
function renderTooltip(target, step) {
...
function renderErrorTooltip(step) {
    const shadow = getShadow();
    removeTooltip();
    const { stepIndex, activeWalkthrough } = useContentStore.getState();
    const total = activeWalkthrough.steps.length;
...
function attachTrigger(el, trigger, onAdvance) {
...
function triggerHint(t) {
    if (t === 'click-target')
        return 'Click the highlighted element to advance';
    if (t === 'input-change')
        return 'Update the field value to advance';
    return '';
}
